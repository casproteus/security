...
public class HangupDlg extends JDialog implements ICASDialog, ActionListener, ComponentListener, KeyListener {
    ...
    public void actionPerformed(
        ...
        if (tSelectedRow < tblContent.getRowCount() && tSelectedRow >= 0) {
            PosFrame.instance.general.cancelHangup(tSelectedRow);
        }
        dispose();
    }
    ...
    private void initTable() {
        Vector tHangupVec = PosFrame.instance.general.hangupVec;
        ...
        for (int i = 0, len = tHangupVec.size(); i < len; i++) {
            ...
            for (int j = 0; j < header.length; j++)
                tValues[i][j] =
                        j == 0 ? PosDlgConst.Customer.concat(String.valueOf(i + 1)) : (tValueHangedup.length < j ? null
    ...
    private String[] header = new String[] { " ", PosDlgConst.Product.concat("1"), PosDlgConst.Product.concat("2"),
            PosDlgConst.Product.concat("3"), PosDlgConst.Product.concat("4") };
    PIMTable tblContent;
