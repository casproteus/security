...
public class ImportanceRenderer extends DefaultPIMTableCellRenderer
{
    /** 创建一个<CODE>ContactsIconRenderer</CODE>的实例
     */
    public ImportanceRenderer ()
    {
        super();
        //目前规格只用到一个图标
    ...
    boolean isSelected, boolean hasFocus, int row, int column)
    {
        ...
        else if (value instanceof Item)
        {
            ...
            if (str.equals(MailOptionConstant.IMPORTANT[0]))
            {
                setIcon(iconImportance);
            }
            else if (str.equals(MailOptionConstant.IMPORTANT[1]))
            {
                setIcon(null);
            }
            else if (str.equals(MailOptionConstant.IMPORTANT[2]))
            {
