...
public class MagicbathTiedView extends AbstractPIMView implements Releasable {
    private static MagicbathTiedView instance;

    public static MagicbathTiedView getInstance() {
        if (instance == null)
            instance = new MagicbathTiedView();
        return instance;
    }

    private MagicbathTiedView() {
    ...
    public void init(
        ...
        if (CustOpts.custOps.isPreviewShown()) {
            ...
            if (prmAppChanged) {// 检查预览面板上的内容是否已准备好了，没有的话先将其初始化好。
                ...
                dialog = MainPane.getApp("Magicbath").getADialog();
                dialog.getContainer().setBackground(Color.lightGray);
                add(dialog.getContainer());
            }
        }
        if (tmpViewInfo.getPathID() == 5400) {
        ...
        panelAry = new BathBoxLable[contents.length];
        panel.removeAll();
        for (int i = 0, len = panelAry.length; i < len; i++) {
            panelAry[i] =
                    new BathBoxLable(((Integer) contents[i][0]).intValue(), (String) contents[i][1], contents[i][2]);
