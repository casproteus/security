 ...
 */
public class InputDlg extends JDialog implements ICASDialog, ActionListener, WindowListener, ComponentListener {
    /**
     ...
     *            弹出对话框的Frame
     */
    public InputDlg(Frame parent, ActionListener prmAction, PIMRecord prmRecord) {
        super(parent, false);
     ...
     *            弹出对话框的Frame
     */
    public InputDlg(JDialog parent, ActionListener prmAction, PIMRecord prmRecord) {
        super(parent, true);
        actionListener = prmAction; // 当对话盒采集输入结束后，调该监听的actionperform方法。
        record = prmRecord;
        initDialog();
     ...
     *            OK时的操作
     */
    public InputDlg(JDialog parent, ActionListener prmAction) {
        super(parent, true);
        actionListener = prmAction; // 当对话盒采集输入结束后，调该监听的actionperform方法。
        record = null;
        initDialog();
    ...
    public PIMRecord getContents() {
        general.saveData();
        record.setAppIndex(CustOpts.custOps.APPNameVec.indexOf("Input"));
        return record;
    }
    ...
    public void actionPerformed(
        ...
        if (e.getSource() == ok) {
            ...
            tmpTextInField = general.fldOthercost.getText();
            if (!tmpTextInField.equals(CASUtility.EMPTYSTR))
                try {
                    Double.valueOf(tmpTextInField);
                } catch (NumberFormatException exp) {
                    general.fldOthercost.grabFocus();
                    general.fldOthercost.selectAll();
                    JOptionPane.showMessageDialog(this, DlgConst.FORMATERROR);
                    return;
                }
            if (newFlag) {
            ...
            if (general.isValueChanged() // 具体做法保存之前为检查每个域中的值是否和record中的一致。
                ...
                if (newFlag) {
                    ...
                    if (tree.getNodePath(treePath.getParentPath(), treePath.getLastPathComponent().toString()) != null) // 结点没被删除
                        ...
                        int tmpIndex = CustOpts.custOps.APPNameVec.indexOf("Input");
                        int tmpAppNodeID = CASUtility.getAPPNodeID(tmpIndex);// 得到对于本应用的根本路径。
                        if (CASUtility.setPathFromDialog(this, record, tmpIndex))
                            return; // 选择文件夹点的取消按钮,直接退到联系人对话盒
                        record.setInfolderID(tmpAppNodeID);
    ...
    private void initDialog() {
        ...
        general = new InputGeneralPanel(this);
        ...
        // 搭建－－－－－－－－－－－－－
        tabPane.addTab(InputDlgConst.GENERAL, general);
    ...
    private InputGeneralPanel general;
