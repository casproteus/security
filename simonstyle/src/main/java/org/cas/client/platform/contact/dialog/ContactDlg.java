...
/**
 * 联系人对话框 目前已知在两种情况下会用到该对话盒:1/View上面显示;2/作为单独的对话盒显示. 因为考虑到所有作为单独对话盒显示的情况下,其parent都不应该为null,只有当作为View上的组件显示时,才是
 * 不需要parent的,即parent可赋为null的.所以parent是否为null,就是对话盒是作为View的部分显示还是做 为单独对话和显示的区别标志,布局也将据此决定是否显示确定取消按钮.
 * <p>
 * 微软的TextField的长度限制是228（约）， 本类的限制因为没有规格约束，暂定为912（约）
 */
public class ContactDlg extends JDialog implements ICASDialog, ActionListener, WindowListener, ComponentListener {
    /**
     ...
     *            弹出对话框的Frame
     */
    public ContactDlg(Frame parent, ActionListener prmAction, PIMRecord prmRecord) {
        super(parent, false);
     ...
     *            弹出对话框的Frame
     */
    public ContactDlg(JDialog parent, ActionListener prmAction, PIMRecord prmRecord) {
        super(parent, true);
        actionListener = prmAction; // 当对话盒采集输入结束后，调该监听的actionperform方法。
        record = prmRecord;
        initDialog();
     ...
     *            OK时的操作
     */
    public ContactDlg(JDialog parent, ActionListener prmAction) {
        super(parent, true);
        actionListener = prmAction; // 当对话盒采集输入结束后，调该监听的actionperform方法。
        record = null;
        initDialog();
    ...
    public PIMRecord getContents() {
        general.saveData();
        record.setAppIndex(CustOpts.custOps.APPNameVec.indexOf("Contact"));
        return record;
    }
    ...
    public void actionPerformed(
        ...
        if (e.getSource() == ok) {
            ...
            if (tmpTextInField == null || tmpTextInField.length() <= 0) {
                ...
                // TODO:报错，说明不可以没有必要的联系人字段。
                general.fldLastName.requestFocus();
                return;
            }
            tmpTextInField = general.fldAccount.getText();
            if (!tmpTextInField.equals(CASUtility.EMPTYSTR))
                try {
                    Double.valueOf(tmpTextInField);
                } catch (NumberFormatException exp) {
                    general.fldAccount.grabFocus();
                    general.fldAccount.selectAll();
            ...
            // 如果是新建记录,那么确定后不关闭对话盒,并且在release方法中仅释放对话盒中的内容.不释放对话盒.==============================
            if (newFlag) {
                ...
                SwingUtilities.invokeLater(new Runnable() {
                    public void run() {
                        general.fldCompName.requestFocus();
                    }
                });
            } else
                setVisible(false);
            actionListener.actionPerformed(new ActionEvent(this, 0, null));
        } else if (e.getSource() == cancel) { // 选择关闭按扭，则检查是否存在内容的改变，如果有任何改变则提示用户保存。
            if (general.isValueChanged() // 具体做法保存之前为检查每个域中的值是否和record中的一致。
                    && JOptionPane.showConfirmDialog(this, DlgConst.SAVEBEFORECLOSE,// "系统检测到对话框内的内容发生了改变，是否需要在关闭前保存改变的内容？",
    ...
    /**
     * @Append：因为添加进货记录时，需要增加新供应商。这时，需要构造一个记录，使path指向供应商节点。所以记录不为Null。然而 
    ...
    private void initDialog() {
        newFlag = record == null || record.getFieldValue(ContactDefaultViews.ID) == null;
        setTitle(ContactDlgConst.EMPLOYEE);
        if (newFlag)
        ...
        general = new ContactGeneralPanel(this);
        ...
        // 搭建－－－－－－－－－－－－－
        tabPane.addTab(ContactDlgConst.GENERAL, general);
    ...
    private ContactGeneralPanel general;
