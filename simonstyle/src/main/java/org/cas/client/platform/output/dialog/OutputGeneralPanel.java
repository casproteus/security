...
class OutputGeneralPanel extends JScrollPane implements ActionListener, Runnable, Releasable {
     ...
     *            父窗体
     */
    OutputGeneralPanel(OutputDlg prmDlg) {
        dlg = prmDlg;
    }
    ...
    void init() {
        ...
        tblContent =
                new PIMTable(new PIMTableModelAryBased(tmpValues, new Object[] { ImportDlgConst.TEXTAREA,
                        ImportDlgConst.COMMATION_AREA }), null, null, false);
    ...
    boolean isValueChanged() {
        ...
        if (tmpValue1 != null && tmpValue1 instanceof Date)// 生成文本
            tSeleDate =
                    new StringBuffer().append(((Date) tmpValue1).getYear() + 1900).append('-')
                            .append(((Date) tmpValue1).getMonth() + 1).append('-').append(((Date) tmpValue1).getDate())
                            .append(CASUtility.SPACE).toString();
        if ((tmpValue1 != null && tmpValue2 == null)
                || (tmpValue1 == null && tmpValue2 != null)
                || (tmpValue1 instanceof Date && tmpValue2 instanceof String && !((String) tmpValue2)
                        .startsWith(tSeleDate)))
            return true;
        // 顾客ID--------------------------------------
        tmpValue1 =
                cmdContact.getSelectedIndex() >= 0 ? Integer.valueOf(contactIDAry[cmdContact.getSelectedIndex()]) : "";
    ...
    void reLayout() {
        ...
        final int temLableWidthLeft =
                CASDialogKit.getMaxWidth(new JComponent[] { lblCode, lblName, lblAmount, lblTime, lblEmployee,
                        lblTotlePrice })
                        + CustOpts.HOR_GAP;
        ...
        if (tmpIsVerLayout) {
            ...
            cmdEmployee.setBounds(lblEmployee.getX() + temLableWidthLeft, lblEmployee.getY(), temFieldWidthLeft,
                    CustOpts.BTN_HEIGHT);
            // 产品名称
            lblName.setBounds(lblEmployee.getX(), lblEmployee.getY() + CustOpts.BTN_HEIGHT + CustOpts.VER_GAP,
                    temLableWidthLeft, CustOpts.BTN_HEIGHT);
            cmbProdName.setBounds(lblName.getX() + temLableWidthLeft, lblName.getY(), temFieldWidthLeft,
                    CustOpts.BTN_HEIGHT);
            // 数量
            lblAmount.setBounds(lblName.getX(), lblName.getY() + CustOpts.BTN_HEIGHT + CustOpts.VER_GAP,
                    temLableWidthLeft, CustOpts.BTN_HEIGHT);
            ...
            fldTotlePrice.setBounds(lblTotlePrice.getX() + temLableWidthLeft, lblTotlePrice.getY(), temFieldWidthLeft,
                    CustOpts.BTN_HEIGHT);
            // 客户
            lblContact.setBounds(lblTotlePrice.getX(), lblTotlePrice.getY() + CustOpts.BTN_HEIGHT + CustOpts.VER_GAP,
                    temLableWidthLeft, CustOpts.LBL_HEIGHT);
            cmdContact.setBounds(lblContact.getX() + temLableWidthLeft, lblContact.getY(), temFieldWidthLeft,
                    CustOpts.BTN_HEIGHT);
            // 欠款
            lblArrearage.setBounds(lblContact.getX(), lblContact.getY() + CustOpts.BTN_HEIGHT + CustOpts.VER_GAP,
                    temLableWidthLeft, CustOpts.LBL_HEIGHT);
            ...
            fldTotlePrice.setBounds(lblTotlePrice.getX() + temLableWidthLeft, lblTotlePrice.getY(), temFieldWidthLeft,
                    CustOpts.BTN_HEIGHT);
            // 客户
            lblContact.setBounds(lblEmployee.getX(), lblEmployee.getY() + CustOpts.BTN_HEIGHT + CustOpts.VER_GAP,
                    temLableWidthLeft, CustOpts.LBL_HEIGHT);
            cmdContact.setBounds(lblContact.getX() + temLableWidthLeft, lblContact.getY(), temFieldWidthLeft,
                    CustOpts.BTN_HEIGHT);
            // 欠款
