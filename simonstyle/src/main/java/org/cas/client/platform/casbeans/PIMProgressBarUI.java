...
public class PIMProgressBarUI extends ProgressBarUI implements ChangeListener {
    ...
    protected PIMProgressBar progressBar;

    // ComponentUI
    public static ComponentUI createUI(
            JComponent x) {
        return new PIMProgressBarUI();
    }

    // installUI
    public void installUI(
            JComponent c) {
        progressBar = (PIMProgressBar) c;
        installDefaults();
        installListeners();
    }
    ...
    private void setCachedPercent(
            int cachedPercent) {
        this.cachedPercent = cachedPercent;
    }

    /**
     * Returns the width (if HORIZONTAL) or height (if VERTICAL) of each of the indivdual cells/units to be rendered in
     * the progress bar. However, for text rendering simplification and aesthetic considerations, this function will
    ...
    public void setCellLength(
            int cellLen) {
        this.cellLength = cellLen;
    }

    /**
     * Returns the spacing between each of the cells/units in the progress bar. However, for text rendering
     * simplification and aesthetic considerations, this function will return 0 when the progress string is being
     * rendered.
     *
     * @return the value representing the spacing between cells
     * @see #setCellSpacing
    ...
    /**
     * This determines the amount of the progress bar that should be filled based on the percent done gathered from the
     * model. This is a common operation so it was abstracted out. It assumes that your progress bar is linear. That is,
     * if you are making a circular progress indicator, you will want to override this method.
     */
    ...
    /**
     * All purpose paint method that should do the right thing for almost all linear progress bars. By setting a few
     * values in the defaults table, things should work just fine to paint your progress bar. Naturally, override this
     * if you are making a circular or semi-circular progress bar.
     */
    public void paint(
            Graphics g,
            JComponent c) {
        ...
        if (progressBar.getOrientation() == JProgressBar.HORIZONTAL) {
            ...
            if (cellSpacing == 0 && amountFull > 0) {
                ...
                for (current = fillX; numCells-- > 0; current += increment) {
                    // + cellLength +"  barRectHeight "+ barRectHeight);
                    g.fillRect(current, barRectY + 1, cellLength, barRectHeight - 2);
                }
                g.setClip(oldClip);
            }
        } else {
    ...
    /**
     * Designate the place where the progress string will be drawn. This implementation places it at the center of the
    ...
    protected Point getStringPlacement(
        ...
        if (progressBar.getOrientation() == JProgressBar.HORIZONTAL) {
            return new Point(x + Math.round(width / 2 - stringWidth / 2), y + fontSizer.getHeight()
    ...
    /**
     * The Minimum size for this component is 10. The rationale here is that there should be at least one pixel per 10
     * percent.
     */
     ...
     * &quot;protected&quot; inner class. Instantiate it only within subclasses of BasicProgressBarUI.
     */
    // stateChanged
