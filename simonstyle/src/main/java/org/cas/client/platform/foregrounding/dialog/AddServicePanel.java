...
public class AddServicePanel extends JTabbedPane implements ComponentListener, ActionListener {
    public AddServicePanel() {
        // init-----------
        general = new JPanel();
        lblEmployee = new JLabel(ForegroundingDlgConst.SeleEmployee);
        cmbEmployee = new JComboBox();
        lblSeleService = new JLabel(ForegroundingDlgConst.SeleService);
        cmbService = new JComboBox();
        lblSeleBoxNumbers = new JLabel(ForegroundingDlgConst.SeleBoxNumber);
        cmbBoxNumbers = new JComboBox();
        lblCount = new JLabel(ForegroundingDlgConst.ProdCount);
        fldCount = new JTextField();
        ok = new JButton(ForegroundingDlgConst.AddIn);
        ...
        general.add(ok);
        addTab(ForegroundingDlgConst.RecordService, general);
    ...
    public void actionPerformed(
        ...
        if (tSource == ok) {
            ...
            if (tmpStr == null || tmpStr.length() == 0) {
                JOptionPane.showMessageDialog(this, ForegroundingDlgConst.MsgSelectEmployee);
                cmbEmployee.requestFocus();
                return;
            }
            tmpStr = (String) cmbService.getSelectedItem();
            if (tmpStr == null || tmpStr.length() == 0) {
                JOptionPane.showMessageDialog(this, ForegroundingDlgConst.MsgSelectService);
                cmbService.requestFocus();
                return;
            }
            tmpStr = (String) cmbBoxNumbers.getSelectedItem();
            if (tmpStr == null || tmpStr.length() == 0) {
                JOptionPane.showMessageDialog(this, ForegroundingDlgConst.MsgSeleBoxNumber);
                cmbBoxNumbers.requestFocus();
                return;
            }
            tmpStr = fldCount.getText();
            if (tmpStr == null || tmpStr.length() < 1) {
                JOptionPane.showMessageDialog(this, ForegroundingDlgConst.MsgInputCount);
