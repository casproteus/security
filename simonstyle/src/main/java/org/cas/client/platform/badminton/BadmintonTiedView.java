...
public class BadmintonTiedView extends AbstractPIMView implements Releasable {
    private static BadmintonTiedView instance;

    public static BadmintonTiedView getInstance() {
        if (instance == null)
            instance = new BadmintonTiedView();
        return instance;
    }

    private BadmintonTiedView() {
    ...
    public void setBounds(
        ...
        if (CustOpts.custOps.isBaseBookHide()) {// 非Book风格。
            int tTitlePaneHeight = 0;
            if (CustOpts.custOps.isPreviewShown()) {
                int tmpPos = CustOpts.custOps.getSplitHeight();
                if (CustOpts.custOps.isViewTopAndDown()) {
                    scrollPane.setBounds(0, tTitlePaneHeight, prmWidth, tmpPos - tTitlePaneHeight);
                    layoutGeneral();
                    getDlg().getContainer().setBounds(0, tmpPos + MainPane.getDividerSize(), prmWidth,
                            prmHeight - tmpPos - MainPane.getDividerSize());
                    getDlg().reLayout();
                } else {
                    scrollPane.setBounds(0, tTitlePaneHeight, tmpPos, prmHeight - tTitlePaneHeight);
                    layoutGeneral();
                    getDlg().getContainer().setBounds(tmpPos + MainPane.getDividerSize(), tTitlePaneHeight,
                            prmWidth - tmpPos - MainPane.getDividerSize(), prmHeight - tTitlePaneHeight);
                    getDlg().reLayout();
                }
                titlePane.setBounds(0, 0, prmWidth, tTitlePaneHeight);
                titlePane2.setBounds(0, 0, 0, 0);
            } else {
                titlePane.setBounds(0, 0, prmWidth, tTitlePaneHeight);
                scrollPane.setBounds(0, tTitlePaneHeight, prmWidth, prmHeight - tTitlePaneHeight);
                layoutGeneral();
                titlePane2.setBounds(0, 0, 0, 0);
                if (dialog != null)
    ...
    private void layoutGeneral() {
        if (panelAry == null || panelAry.length < 1)
            return;
        int tPWidth = scrollPane.getWidth() - 25;
        int tLWidth = panelAry[0].getWidth();
        int tLHeight = panelAry[0].getHeight();
        int tCount = (int) tPWidth / tLWidth;
        if (tCount < 1)
            return;
        int tGap = (int) (tPWidth - tLWidth * tCount) / tCount + 1;
        for (int i = 0, len = panelAry.length; i < len; i++)
            panelAry[i].setBounds((i % tCount) * (tLWidth + tGap) + tGap, (int) (i / tCount) * (tLHeight + tGap),
                    tLWidth, tLHeight);
        panel.setPreferredSize(new Dimension(scrollPane.getWidth(), panelAry[panelAry.length - 1].getY() + tLHeight));
    }
    ...
    public void init(
        ...
        if (CustOpts.custOps.isPreviewShown()) {
            ...
            if (prmAppChanged) {// 检查预览面板上的内容是否已准备好了，没有的话先将其初始化好。
                ...
                dialog = MainPane.getApp("Badminton").getADialog();
                dialog.getContainer().setBackground(Color.lightGray);
                add(dialog.getContainer());
            }
        }
        if (tmpViewInfo.getPathID() == 5800) {
            tmpViewInfo.setFolderID(5102);
            tmpViewInfo.setFieldNames("0,3,4");
        }
        contents = tmpModel.selectRecords(tmpViewInfo); // 得到准备要显示的内容，TODO：要么对该方法返回的数组做尺寸限制，要么尝试改到绘制的时候再显示。
        panelAry = new PlatLable[contents.length];
        panel.removeAll();
        for (int i = 0, len = panelAry.length; i < len; i++) {
            panelAry[i] = new PlatLable(((Integer) contents[i][0]).intValue(), (String) contents[i][1], contents[i][2]);
            panel.add(panelAry[i]);
        }
    }
    ...
    public void resetState(
        ...
        for (int i = 0; i < panelAry.length; i++)
            if (panelAry[i].resetState(pSubject, pTime))
                break;
    }
