...
public class AddStoreDlg extends JDialog implements ICASDialog, ActionListener, ComponentListener, FocusListener,
            ...
            if (tKeyCode == 10) { // 货号框中，随时当按回车键，立即开始在库中查询匹配商品。（通常执行不到，因为扫描输入后，会根据位数判断自动执行查询。
                ...
                try {
                    ...
                    if (rs.getRow() > 0) {
                        ...
                        MerchandiseDlg tDlg =
                                new MerchandiseDlg(PosFrame.instance, -1, tProdNumber, CASUtility.getPriceByCent(Double
    ...
    public void keyReleased(
            KeyEvent e) {
        if (PosUtility.isNumber(e.getKeyCode())) {
            Object o = e.getSource();
            if (o == tfdCost) {
    ...
    private void initDialog() {
        ...
        lblCost = new JLabel(PosDlgConst.Cost);
        ...
        lblSupplier = new JLabel(PosDlgConst.Supplier);
    ...
    private void initComponents() {
        ...
        try {
            ...
            supplierSubjectAry[tmpPos] = PosDlgConst.AddNewSupplier;
            rs.close();// 关闭
    ...
    private String[] header = new String[] { PosDlgConst.ProdNumber, PosDlgConst.ProdName, PosDlgConst.Count,
            PosDlgConst.Cost, PosDlgConst.Subtotal };
