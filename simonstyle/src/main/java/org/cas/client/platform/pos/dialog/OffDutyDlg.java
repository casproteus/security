...
public class OffDutyDlg extends JDialog implements ICASDialog, ActionListener, ComponentListener, FocusListener,
    ...
    public void keyReleased(
            KeyEvent e) {
        int tKeyCode = e.getKeyCode();
        if (PosUtility.isNumber(tKeyCode)) {
            Object o = e.getSource();
            if (o == tfdRMB) {
    ...
    public void actionPerformed(
        ...
        if (o == ok) {
            ...
            try {
                tChange = Integer.parseInt((String) CustOpts.custOps.getValue(PosDlgConst.Shoestring));
            } catch (Exception exp) {
            }
            ...
            int tReceived =
                    (int) ((tMoneyRMB // 实收金额（float被强制转换成以分为单位的int）。
                            + tMoney0
                            / Float.parseFloat((String) CustOpts.custOps.getValue(PosDlgConst.MoneyUnit[1]))
                            + tMoney1
                            / Float.parseFloat((String) CustOpts.custOps.getValue(PosDlgConst.MoneyUnit[2]))
                            + tMoney2
                            / Float.parseFloat((String) CustOpts.custOps.getValue(PosDlgConst.MoneyUnit[3]))
                            + tMoney3
                            / Float.parseFloat((String) CustOpts.custOps.getValue(PosDlgConst.MoneyUnit[4]))
                            + tMoney4
                            / Float.parseFloat((String) CustOpts.custOps.getValue(PosDlgConst.MoneyUnit[5]))
                            + tMoney5
                            / Float.parseFloat((String) CustOpts.custOps.getValue(PosDlgConst.MoneyUnit[6]))
                            + tMoney7 / Float.parseFloat((String) CustOpts.custOps.getValue(PosDlgConst.MoneyUnit[7])) + tMoney9
                            / Float.parseFloat((String) CustOpts.custOps.getValue(PosDlgConst.MoneyUnit[8]))) * 100)
                            - tChange;
            ...
            sql =
                    "INSERT INTO evaluation(startTime, endTime, SUBJECT, receive, target, profit) VALUES ('"
                            .concat(PosGeneralPanel.startTime.concat("', '")
                                    .concat(Calendar.getInstance().getTime().toLocaleString()).concat("', '")
            ...
            // 更新新的零钱数进入ini文件。
            CustOpts.custOps.setKeyAndValue(PosDlgConst.Shoestring, String.valueOf(tReceived + tChange));
            // 显示信息
            JOptionPane.showMessageDialog(this, PosDlgConst.GoodBye);
            PosFrame.instance.setVisible(false);
    ...
    private void initDialog() {
        setTitle(PosDlgConst.OffDuty);
        setResizable(false);
        setBounds((CustOpts.SCRWIDTH - 200) / 2, (CustOpts.SCRHEIGHT - 330) / 2, 200, 330); // 对话框的默认尺寸。

        // 初始化－－－－－－－－－－－－－－－－
        lblRMB = new JLabel(PosDlgConst.MoneyUnit[0].concat("："));
        lblForeign0 = new JLabel(PosDlgConst.MoneyUnit[1].concat("："));
        lblForeign1 = new JLabel(PosDlgConst.MoneyUnit[2].concat("："));
        lblForeign2 = new JLabel(PosDlgConst.MoneyUnit[3].concat("："));
        lblForeign3 = new JLabel(PosDlgConst.MoneyUnit[4].concat("："));
        lblForeign4 = new JLabel(PosDlgConst.MoneyUnit[5].concat("："));
        lblForeign5 = new JLabel(PosDlgConst.MoneyUnit[6].concat("："));
        lblForeign6 = new JLabel(PosDlgConst.MoneyUnit[7].concat("："));
        lblForeign7 = new JLabel(PosDlgConst.MoneyUnit[8].concat("："));
