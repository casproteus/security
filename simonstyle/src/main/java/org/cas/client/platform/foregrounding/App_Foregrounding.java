...
public class App_Foregrounding extends AbstractApp {
    /** Creates a new instance of TaskViewPane */
    public App_Foregrounding() {
        super(null);
        initActionFlags();
    }
    ...
    public void initInfoInDB() {
        for (int j = 0; j < ForegroundingDefaultViews.INIT_DB_VIEWINFO.length; j++)
            try {
                ...
                stmt.executeUpdate(ForegroundingDefaultViews.INIT_DB_VIEWINFO[j]);
            } catch (Exception exp) {
    ...
    public IView getTiedView() {
        return new ForegroundingTiedView();
    }
