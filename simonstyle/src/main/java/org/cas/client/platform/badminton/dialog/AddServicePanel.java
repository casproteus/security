...
public class AddServicePanel extends JTabbedPane implements ComponentListener, ActionListener {
    public AddServicePanel() {
        // init-----------
        general = new JPanel();
        lblEmployee = new JLabel(BadmintonDlgConst.SeleEmployee);
        cmbEmployee = new JComboBox();
        lblSeleService = new JLabel(BadmintonDlgConst.SeleService);
        cmbService = new JComboBox();
        lblSeleBoxNumbers = new JLabel(BadmintonDlgConst.SeleBoxNumber);
        cmbBoxNumbers = new JComboBox();
        lblCount = new JLabel(BadmintonDlgConst.Times);
        ...
        general.add(ok);
        addTab(BadmintonDlgConst.RecordService, general);
    ...
    public void actionPerformed(
        ...
        if (tSource == ok) {
            ...
            if (tmpStr == null || tmpStr.length() == 0) {
                JOptionPane.showMessageDialog(this, BadmintonDlgConst.MsgSelectEmployee);
                cmbEmployee.requestFocus();
                return;
            }
            tmpStr = (String) cmbService.getSelectedItem();
            if (tmpStr == null || tmpStr.length() == 0) {
                JOptionPane.showMessageDialog(this, BadmintonDlgConst.MsgSelectService);
                cmbService.requestFocus();
                return;
            }
            tmpStr = (String) cmbBoxNumbers.getSelectedItem();
            if (tmpStr == null || tmpStr.length() == 0) {
                JOptionPane.showMessageDialog(this, BadmintonDlgConst.MsgSeleBoxNumber);
                cmbBoxNumbers.requestFocus();
                return;
            }
            tmpStr = fldCount.getText();
            if (tmpStr == null || tmpStr.length() < 1) {
                JOptionPane.showMessageDialog(this, BadmintonDlgConst.MsgInputCount);
