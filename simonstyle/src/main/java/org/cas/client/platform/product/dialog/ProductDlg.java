 ...
 */
public class ProductDlg extends JDialog implements ICASDialog, ActionListener, WindowListener, ComponentListener {

    /**
     * Creates a new instance of ProductDialog
     * 
     * @param prmAction
     *            OK时的操作
     * @param prmRecord
     ...
     *            弹出对话框的Frame
     */
    public ProductDlg(Frame parent, ActionListener prmAction, PIMRecord prmRecord) {
        super(parent, false);
     ...
     *            弹出对话框的Frame
     */
    public ProductDlg(JDialog parent, ActionListener prmAction, PIMRecord prmRecord) {
        super(parent, true);
        actionListener = prmAction; // 当对话盒采集输入结束后，调该监听的actionperform方法。
        record = prmRecord;
        initDialog();
    ...
    public PIMRecord getContents() {
        general.saveData();
        record.setAppIndex(CustOpts.custOps.APPNameVec.indexOf("Product"));
        return record;
    }
     ...
     * 实现接口中的方法
     * 
     * @called by:emo.pim.pimcontrol.action.pimmenu.file.ProductAciton;
     */
    public void release() {
        if (newFlag) {
            setContents(null);
    ...
    public void actionPerformed(
        ...
        if (e.getSource() == ok) {
            // 先检查价格的输入是否有效===========================================================================
            String tmpTextInField = general.fldPrice.getText();
            try {
                Double.valueOf(tmpTextInField);
            } catch (NumberFormatException exp) {
                general.fldPrice.grabFocus();
                general.fldPrice.selectAll();
            ...
            if (newFlag) {
                ...
                if (tree.getNodePath(treePath.getParentPath(), treePath.getLastPathComponent().toString()) != null) // 结点没被删除
                    ...
                    int tmpIndex = CustOpts.custOps.APPNameVec.indexOf("Product");
            ...
            if (general.isValueChanged()
                ...
                tmpTextInField = general.fldTimes.getText();
                try {
                    Integer.valueOf(tmpTextInField);
                } catch (NumberFormatException exp) {
                    general.fldTimes.grabFocus();
                    general.fldTimes.selectAll();
                    JOptionPane.showMessageDialog(this, DlgConst.FORMATERROR);
                    return;
                }
                if (newFlag) {
    ...
    /**
     * 仅仅为了“表示为comboBox”收集数据而提供
     * 
     * @called by: ProductDetailPanel;
     */
    ProductGeneralPanel getProductGeneralPanel() {
        return general;
    }
    ...
    private void initDialog() {
        ...
        // Title,bounds－－－－－－
        setTitle(ProductDlgConst.TITLE);
        if (newFlag)
        ...
        general = new ProductGeneralPanel(this);
        ...
        // 搭建－－－－－－－－－－－－－
        tabPane.addTab(ProductDlgConst.GENERAL, general);
    ...
    private JTabbedPane tabPane; // tab选项卡
    private ProductGeneralPanel general;
