...
public class PIMScrollPaneLayout implements LayoutManager, ScrollPaneConstants, Serializable {

    /**
     * The scrollpane's viewport child. Default is an empty <code>JViewport</code>.
     * 
     * @see PIMScrollPane#setViewport
     */
     ...
     * 
     * @see PIMScrollPane#setVerticalScrollBar
     */
     ...
     * 
     * @see PIMScrollPane#setHorizontalScrollBar
     */
     ...
     * 
     * @see PIMScrollPane#setRowHeader
     */
     ...
     * 
     * @see PIMScrollPane#setColumnHeader
     */
     ...
     * <p>
     * This field is obsolete, please use the <code>PIMScrollPane</code> field instead.
     *
     * @see PIMScrollPane#setVerticalScrollBarPolicy
     */
     ...
     * This field is obsolete, please use the <code>PIMScrollPane</code> field instead.
     *
     * @see PIMScrollPane#setHorizontalScrollBarPolicy
     */
    protected int hsbPolicy = HORIZONTAL_SCROLLBAR_AS_NEEDED;

    /**
     * This method is invoked after the PIMScrollPaneLayout is set as the LayoutManager of a <code>PIMScrollPane</code>.
     * It initializes all of the internal fields that are ordinarily set by <code>addLayoutComponent</code>. For
     * example:
     * 
     * <pre>
     * PIMScrollPaneLayout mySPLayout = new ScrollPanelLayout() {
    ...
    /**
     * Removes an existing component. When a new component, such as the left corner, or vertical scrollbar, is added,
     * the old one, if it exists, must be removed.
     * <p>
     * This method returns <code>newC</code>. If <code>oldC</code> is not equal to <code>newC</code> and is non-
     * <code>null</code>, it will be removed from its parent.
     *
     * @param oldC
     ...
     */
    protected Component addSingletonComponent(
            Component oldC,
            Component newC) {
     ...
     * <ul>
     * <li>PIMScrollPane.VIEWPORT
     * <li>PIMScrollPane.VERTICAL_SCROLLBAR
     * <li>PIMScrollPane.HORIZONTAL_SCROLLBAR
     * <li>PIMScrollPane.ROW_HEADER
     * <li>PIMScrollPane.COLUMN_HEADER
     ...
     * </ul>
     * Note: Applications should use the <code>PIMScrollPane</code> version of this method. It only exists for backwards
     * compatibility with the Swing 1.0.2 (and earlier) versions of this class.
     *
     * @param x
     ...
     * <li>PIMScrollPane.HOTRIZONTAL_SCROLLBAR_NEVER
     ...
     * @see PIMScrollPane#getHorizontalScrollBar
     */
     ...
     * @see PIMScrollPane#getVerticalScrollBar
     */
     ...
     * @see PIMScrollPane#getRowHeader
     */
     ...
     * @see PIMScrollPane#getColumnHeader
     */
    ...
    public Dimension preferredLayoutSize(
            Container parent) {
        /*
         * Sync the (now obsolete) policy fields with the PIMScrollPane.
         */
        ...
        /*
         * Note that viewport.getViewSize() is equivalent to viewport.getView().getPreferredSize() modulo a null view or
         * a view whose size was explicitly set.
         */
        ...
        /*
         * If there's a PIMScrollPane.viewportBorder, add its insets.
         */

        PIMScrollPaneBorder viewportBorder = scrollPane.getViewportBorder();
        ...
        /*
         * If a scrollbar is going to appear, factor its preferred size in. If the scrollbars policy is AS_NEEDED, this
         * can be a little tricky: - If the view is a Scrollable then scrollableTracksViewportWidth and
         * scrollableTracksViewportHeight can be used to effectively disable scrolling (if they're true) in their
         * respective dimensions. - Assuming that a scrollbar hasn't been disabled by the previous constraint, we need
         * to decide if the scrollbar is going to appear to correctly compute the JScrollPanes preferred size. To do
         * this we compare the preferredSize of the viewport (the extentSize) to the preferredSize of the view. Although
         * we're not responsible for laying out the view we'll assume that the JViewport will always give it its
         * preferredSize.
         */
    ...
    public void layoutContainer(
        ...
        Rectangle colHeadR = new Rectangle(0, availR.y - 1, 0, 0);
        ...
        /*
         * If there's a visible row header remove the space it needs from the left or right of availR. The row header is
         * treated as if it were fixed width, arbitrary height.
         */
        ...
        /*
         * If there's a PIMScrollPane.viewportBorder, remove the space it occupies for availR.
         */
        ...
        /*
         * At this point availR is the space available for the viewport and scrollbars. rowHeadR is correct except for
         * its height and y and colHeadR is correct except for its width and x. Once we're through computing the
         * dimensions of these three parts we can go back and set the dimensions of rowHeadR.height, rowHeadR.y,
         * colHeadR.width, colHeadR.x and the bounds for the corners. We'll decide about putting up scrollbars by
         ...
         * next. And we assume that the viewports layout manager will give the view it's preferred size. One exception
        ...
        /*
         * If there's a vertical scrollbar and we need one, allocate space for it (we'll make it visible later). A
         * vertical scrollbar is considered to be fixed width, arbitrary height.
         */

        // Rectangle vsbR = new Rectangle(0, availR.y - vpbInsets.top, 0, 0);
        int headerHeight = availR.y - vpbInsets.top;
        ...
        /*
         * If there's a horizontal scrollbar and we need one, allocate space for it (we'll make it visible later). A
         * horizontal scrollbar is considered to be fixed height, arbitrary width.
         */
        ...
        if ((hsb != null) && hsbNeeded) {
            ...
            /*
             * If we added the horizontal scrollbar then we've implicitly reduced the vertical space available to the
             * viewport. As a consequence we may have to add the vertical scrollbar, if that hasn't been done so
             * already. Of course we don't bother with any of this if the vsbPolicy is NEVER.
             */
        ...
        /*
         * Set the size of the viewport first, and then recheck the Scrollable methods. Some components base their
         * return values for the Scrollable methods on the size of the Viewport, so that if we don't ask after resetting
        ...
        /*
         * We now have the final size of the viewport: availR. Now fixup the header and scrollbar widths/heights.
         */
        vsbR.height = availR.height + vpbInsets.top + vpbInsets.bottom + headerHeight;
        ...
        /*
         * Set the bounds of the remaining components. The scrollbars are made invisible if they're not needed.
         */
     ...
     * @deprecated As of JDK version Swing1.1 replaced by <code>PIMScrollPane.getViewportBorderBounds()</code>.
     * @param scrollpane
     *            滚动面板
     */
    ...
    /**
     * The UI resource version of <code>PIMScrollPaneLayout</code>.
     */
    public static class UIResource extends PIMScrollPaneLayout implements javax.swing.plaf.UIResource {
    }
}
