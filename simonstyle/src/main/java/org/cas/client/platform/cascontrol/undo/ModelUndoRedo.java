...
public class ModelUndoRedo {
    private static ModelUndoRedo instance;

    public static ModelUndoRedo getInstance() {
        if (instance == null)
            instance = new ModelUndoRedo();
        return instance;
    }
    ...
    private ICASModel model = CASControl.ctrl.getModel(); // 执行操作的model
    private UndoableEdit undoEdit; // 用于Undo的UndoableEdit
}
