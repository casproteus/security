 ...
 * Tale风格视图类.
 * 
 * @NOTE:本类作为子组件viewport/及其子组件table的MouseListener,而非自身的鼠标时间监听者. 关于table上双击打开项目的处理在并不在本类中.本类仅负责将事件向外传递至application实例.
 *                                                              本类作为viewport,talble,击快速编辑栏的焦点监听器.
 */

public class View_PIMDetails extends AbstractPIMView implements MouseListener, FocusListener, KeyListener,
    ...
    public View_PIMDetails(String prmHasEditor) {
        ...
        // //table的属性设置结束－－－－－－－－－－
        // table2.setView(this);
        // table2.setAutoResizeMode(PIMTable.AUTO_RESIZE_OFF);
        // table2.setToggleRow(-1000);
        // table2.getTableHeader().setReorderingAllowed(false);//设置表格不允许拖拉
        // //table2的属性设置结束－－－－－－－－－－
        ...
        // table2.addComponentListener(this);
        // table2.addHeadFocusListener(this);
        // 监听器添加完成－－－－－－－－－－
    }
    ...
    public void updatePIMUI() {
        // A.s("updating.......");
        //
        // currentViewInfo = application.getActiveViewInfo(); //得到当前的视图风格。
        //
        // int appType = currentViewInfo.getAppType();
        // String folderPath = currentViewInfo.getFolderPath();
        // model = PIMControl.ctrl.getModel();
        //
        // // currentViewInfo = application.getActiveViewInfo(); //得到当前的视图风格。
        //
        // displayIndexes = model.getFieldNameIndex(currentViewInfo);
        // displayWidths = model.getFieldWidths(currentViewInfo);
        // headerTitles = model.getDisplayObject(currentViewInfo);
        //
        // contents = model.getRecords(currentViewInfo); //得到准备要显示的内容，TODO：要么对该方法返回的数组做尺寸限制，要么尝试改到绘制的时候再显示。
        // if (appType == ModelConstants.CONTACT_APP
        // && currentViewInfo.getFolderPath().indexOf(PaneConstant.SYS_CONT__PATH)!= -1)
        // {
        // Object[][] ecs_records = PIMContain.getDefault().getConnection();
        // if (ecs_records != null)
        // {
        // int length = ecs_records.length;
        // int row_count = length; //contents.length;//
        // int col_count = ModelDBConstants.CER + 1;
        // Object[][] value = new Object[row_count][col_count];
        // for (int idx = 0; idx < row_count; idx++)
        // {
        // {
        // for (int col = 0; col < ecs_records[0].length; col++)
        // {
        // value[idx][col] = ecs_records[idx - contents.length][col];
        // }
        // }
        // }
        // contents = value;
        ...
        //
        //
        // tableContents = PIMViewUtil.getDisplayRecordArray(contents, displayIndexes.length);
        // table.initHeaderSortStatus();
        // // 这里视图中所获得的信息有误,得通知老赵
        // table.setDataVector(contents, headerTitles);
        //
        // //处理行高问题
        // processTableRowHeight();
        // //处理表格中的存放编辑后的值的Vector
        // Vector tmpVe = table.getEditingRowRecords();
        // if (tmpVe != null)
        // {
        // for (int i = 0; i < table.getColumnCount(); i++)
        // {
        // tmpVe.add(i, null);//为什么不能用clear方法？
        // }
        // }
        // //表格体可编辑的控制在这里TODO:为什么不直接从viewInfo中取信息，而要写死？
        // int tmpAppType = getApplication().getActiveViewInfo().getAppType();
        // if (tmpAppType == ModelConstants.LOCAL_APP
        // || tmpAppType == ModelConstants.OUTBOX_APP || tmpAppType == ModelConstants.INBOX_APP
        // || tmpAppType == ModelConstants.SENDED_APP || tmpAppType == ModelConstants.DELETED_ITEM_APP
        // || tmpAppType == ModelConstants.CALENDAR_APP || tmpAppType == ModelConstants.DIARY_APP
        // || tmpAppType == ModelConstants.DRAFT_APP)
        // {
        // table.setHasEditor(false);
        // table.setCellEditable(false);
        // }
        // else if (tmpAppType == ModelConstants.CONTACT_APP || tmpAppType == ModelConstants.TASK_APP)
        // {
        // table.setCellEditable(true);
        // }
        //
        // //设置每列宽度。
        // IPIMTableColumnModel tableColumnModel = table.getColumnModel();
        // int columns = displayWidths.length;
        // for (int i = 0; i < columns; i++)
        // {
        // tableColumnModel.getColumn(i).setWidth(displayWidths[i]);
        ...
        //
        //
        // PIMViewUtil.processSpecialFields(table, model.getFieldNames(currentViewInfo), true);
        //
        // //这种情况是表示视图切换时,上一表格视图是带快速编辑栏的
        // //不带快速编辑栏,但表格头却有编辑器 而且新旧视图信息不同
        // //实际的处理应是只要表格的快速编辑栏中有编辑器，
        // //就必须移除
        //
        // if (table.getTableHeader().getCellEditor() != null)
        // {
        // //tableHeaderEditColumn =
        // // table.getTableHeader().getEditingColumn();
        // table.getTableHeader().removeEditor();
        // }
        // //下面是选中相关的了,TODO:其参数应该向其应用索要，（假设其当前选中记录不需要存盘，如果需要存盘（意义不大），则应该跟ViewInfo索要）。
        // // processTableSelectionStatus(tmpSeleRow, selectedCol, selectedRowCount, rowCount, columnCount,
        // tableHeaderEditColumn);
        // int tmpSeleRow = table.getSelectedRow();
        // int selectedCol = table.getSelectedColumn();
        // int tableHeaderEditColumn = table.getTableHeader().getEditingColumn();
        // int selectedRowCount = table.getSelectedRowCount();
        // int rowCount = table.getRowCount();
        // int columnCount = table.getColumnCount();
        // processTableSelectionStatus(tmpSeleRow, selectedCol, selectedRowCount, rowCount, columnCount,
        // tableHeaderEditColumn);
        // oldViewInfo = (PIMViewInfo)currentViewInfo.clone();
        //
        ...
        // if (hasPreview)
        // {
        // double scale = CustomOptions.custOps.getSplitHeightScale();
        // dividerLocation = (int)((1-scale)*CustomOptions.custOps.getFrameHeight());
        // if (dividerLocation < 0)
        // {
        // dividerLocation = 0;
        // }
        // splitPane.setDividerLocation(dividerLocation); //设置分割条的位置
        //
        // oldSelectedRow = table.getSelectedRow();
        // if (oldSelectedRow == -1)
        // {
        // PIMPreviewPane.getInstance().clear();
        // }
        // else
        // {
        // int tmpRecordID = Integer.parseInt(table.getValueAt(oldSelectedRow, 0).toString()); //取出选中行中的记录ID，
        // PIMRecord tmpRec = PIMControl.ctrl.getModel().selectRecord(appType, tmpRecordID,folderPath);
        // PIMPreviewPane.getInstance().setContents(tmpRec);
        // }
        // }
        // // init();
        // // PIMRecord tmpRec;//得到视图上的第一个记录
        // // String path = null;
        // // if (getSelectedRecords().size() > 0) //如果有记录被选中的话，则应该准备好显示附件。
        // // {
        // // tmpRec = (PIMRecord)getSelectedRecords().get(0);//得到第一个选中记录
        // // //组装记录路径
        // // path =
        ...
        // else
        // {
        // if (appType != ModelConstants.LOCAL_APP)
        // {
    ...
    public void updateTableInfo(
        ...
        if (changedType == IView.SORTOR_CHANGED && changedInfo != null && changedInfo instanceof int[]) {
            ...
            // int tmpAppType = viewInfo.getAppType();
            // Object[] tmpFields = null;
            // if (tmpAppType == ModelConstants.OUTBOX_APP
            // || tmpAppType == ModelConstants.INBOX_APP
            // || tmpAppType == ModelConstants.SENDED_APP
            // || tmpAppType == ModelConstants.DELETED_ITEM_APP)
            // {
            // tmpFields = IntlModelConstants.EMAIL_FLDS;
            // }
            // else if (tmpAppType == ModelConstants.CONTACT_APP)
            // {
            // tmpFields = IntlModelConstants.CONTACTS_FIELD;
            // }
            // else if (tmpAppType == ModelConstants.DIARY_APP)
            // {
            // tmpFields = IntlModelConstants.DIARY_FLDS;
            // }
            // else if (tmpAppType == ModelConstants.TASK_APP)
            // {
            // tmpFields = IntlModelConstants.TASK_FLDS;
            // }
    ...
    public Vector getSelectedRecords() {
        ...
        if (tmpRows == null || tmpRows.length == 0)
            ...
            int tmpAppType = getApplication().getActiveViewInfo().getAppIndex();
            // if (currentViewInfo.getAppIndex() == ModelConstants.DELETED_ITEM_APP){
            // int tmpLength = tmpRows.length;
            // String tmpFirstFd = null;
            // if (tmpLength > 0){
            // Object[] ve = contents[tmpRows[0]];
            // int size = ve.length;
            // tmpFirstFd = (String)ve[size - 2];
            // }
            // if (tmpLength == 1){
            // int tmpID = Integer.parseInt(table.getValueAt(tmpRows[0], 0).toString());
            // PIMRecord tmpRecord = tmpModel.selectRecord(ModelConstants.DELETED_ITEM_APP, tmpID,
            // CustOpts.custOps.getActivePathID());
            // tmpResult.addElement(tmpRecord);
            // }else{
            // int count = 0;
            // int[] ids = new int[tmpLength];
            // for (int i = 0; i < tmpLength; i++){
            // Object[] ve = contents[tmpRows[i]];
            // int size = ve.length;
            // String folder = (String)ve[size - 2];
            //
            // if (folder.equalsIgnoreCase(tmpFirstFd)) //同一张表中的记录
            // ids[count++] = Integer.parseInt(table.getValueAt(tmpRows[i], 0).toString());
            // else{
            // tmpResult.addAll(tmpModel.selectRecords(ModelConstants.DELETED_ITEM_APP, ids,
            // CustOpts.custOps.getActivePathID()));
            // tmpFirstFd = folder;
            // ids = new int[tmpLength];
            // count = 0;
            // ids[count++] = Integer.parseInt(table.getValueAt(tmpRows[i], 0).toString());
            // }
            // if (i == tmpLength - 1)
    ...
    public void updateFontsAndColor() {
        ...
        if (tmpAppType == ModelCons.INBOX_APP || tmpAppType == ModelCons.OUTBOX_APP
            ...
            for (int i = 0; i < contents.length; i++) {
                ...
                // if (tmpAppType == ModelConstants.OUTBOX_APP) //处理未发送邮件
                // {
                // key = "1";
                // processASingleRow(i, key);
                // continue;
                // }
                // else if (tmpAppType != ModelConstants.INBOX_APP)//只有收件箱才有已读标记
                // {
                // key = "-1";
                // processASingleRow(i, key);
                // continue;
                ...
                tmpFlag = tmpV[size - 1]; // ModelDBConstants.READED = size - 1
                                          // //最后一个
                // 处理已读,为空是未读的
        ...
        } else if (true)// tmpAppType == ModelConstants.CONTACT_APP)
        {
            ...
            for (int i = 0; i < contents.length; i++) {
                ...
                tmpFlag = tmpV[size - 2]; // ModelDBConstants.READED = size - 2
                                          // //倒数第二个
                // 处理已读,为空是未读的
    ...
    public void mouseReleased(
        ...
        if (table.hasEditor()) // 有编辑栏的话(按目前的规格,有快速编辑栏同时意味着表格体允许编辑),需要检查上面是否有内容在编辑,如果有并且必要的字段有内容,则保存到model.否则报错.
        {
            if (e.getSource() == table) // 如果点在table上,则如果本来在table上编辑,stopEditing方法已被触发过,如果本来在快速编辑栏上编辑,则还没有存盘.
            {
                ...
                // }
                // else 不处理----------------
            } else // if(e.getSource() == scrollPane.getViewport()) 如果是点在ViewPort上,
            {
    ...
    public void propertyChange(
        ...
        // {
        // int tmpLocation = splitPane.getDividerLocation(); //得到分割位置。
        // if (tmpLocation > dividerLocation + 1 || tmpLocation < dividerLocation - 1)//如果位置有超过10个像素的改变，则需要保存预览面板新分隔点位置。
        // {
        // dividerLocation = tmpLocation;
        // int frameHeight = CustomOptions.custOps.getFrameHeight();
        // CustomOptions.custOps.setSplitHeightScale((double)(frameHeight-dividerLocation)/frameHeight);
        // }
        // }
    }
        ...
        if (CustOpts.custOps.isBaseBookHide()) {// 非Book风格。
            ...
            if (CustOpts.custOps.isPreviewShown()) {
                ...
                if (CustOpts.custOps.isViewTopAndDown()) {// 上下结构
                    ...
                    // scrollPane2.setBounds(0, tmpPos + MainPane.getDividerSize() + titlePaneHeight,
                    // prmWidth, prmHeight - tmpPos - MainPane.getDividerSize() - titlePaneHeight);
                } else { // 左右结构
                    titlePane.setBounds(0, 0, prmWidth, titlePaneHeight);
                    scrollPane.setBounds(0, titlePaneHeight, prmWidth, prmHeight - titlePaneHeight);
                    titlePane2.setBounds(0, 0, 0, 0);// tmpPos + MainPane.getDividerSize(), 0, prmWidth - tmpPos -
                                                     // MainPane.getDividerSize(), titlePaneHeight);
                    // scrollPane2.setBounds(tmpPos + MainPane.getDividerSize(), titlePaneHeight,
                    // prmWidth - tmpPos - MainPane.getDividerSize(), prmHeight - titlePaneHeight);
                }
            }
        }
    ...
    private void processTableSelectionStatus(
        ...
        // 这里是第一次进入视图时所进入的
        if (oldViewInfo == null) {
            // TODO：因为规格可能将快速编辑栏去掉，而且由于PIMViewInfo类的enrichFromRS方法中的错误，所以暂时把该段注释掉，以后根据情况删除改段，或纠正
            // PIMViewInfo类的enrichFromRS方法的错误后打开。
            // if (table.hasEditor())
            // {
            // //把焦点放到快速编辑栏中去
            ...
            // table.getTableHeader().initDefaultEditor(1, anchorColumn);
            // table.getTableHeader().getEditorComponent().requestFocus();
            ...
            //
            // IPIMTableCellEditor cellEditor = table.getTableHeader().getCellEditor();
            // cellEditor.getTableCellEditorComponent(table, null, true, 1, anchorColumn);
            // }
            // else
        ...
        // 判断条件:带快速编辑栏,而且选中行小于零
            ...
            // table.getTableHeader().setEditingColumn(anchorColumn);
            // table.getTableHeader().setSelectedColumnIndex(anchorColumn);
            // table.getTableHeader().initDefaultEditor(anchorRow,
            // anchorColumn);
            //
            // table.getTableHeader().getEditorComponent().requestFocus();
            //
            // IPIMTableCellEditor cellEditor =
            // table.getTableHeader().getCellEditor();
            // cellEditor.getTableCellEditorComponent(table,null,true,anchorRow,anchorColumn);
            //
            // table.getTableHeader().getEditorComponent().invalidate();
            // table.getTableHeader().editCellAt(anchorRow, anchorColumn);
            dealType = 1;
        }
        ...
        // 这种情况表示是切换到没有快速编辑栏的表格视图,默认定焦点到第一行第一列
            ...
            if (table.getRowCount() > 0) {
                // //从有快速编辑栏的视图到没有快速编辑栏的表格视图,十有八九走这里
                // int tmpLocationCol = 1;
                // if (table.hasEditor())
                // {
                // tmpLocationCol =
                // PIMViewUtil.getFirstTextableColumn(table.getTableHeader());
                // }
                table.getSelectionModel().setSelectionInterval(0, 0);
                table.getColumnModel().getSelectionModel().setSelectionInterval(1, 1);
            } else
