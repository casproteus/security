...
public class CalculatePanel extends JTabbedPane implements ComponentListener, ActionListener {
    public CalculatePanel() {
        // init-----------
        general = new JPanel();
        lblSeleBoxNumber = new JLabel(BadmintonDlgConst.SeleBoxNumber);
        cmbBoxNumber = new JComboBox();
        tblContent = new PIMTable();// 显示字段的表格,设置模型
        srpContent = new PIMScrollPane(tblContent);
        ok = new JButton(BadmintonDlgConst.Calculate);
        ...
        general.add(ok);
        addTab(BadmintonDlgConst.CustCalculate, general);
    ...
    public void actionPerformed(
        ...
        if (tSource == ok) {
            if (tblContent.getValueAt(0, 0) == null) { // the first cell is null, means the table is empty.
                JOptionPane.showMessageDialog(this, BadmintonDlgConst.NoUserSelected);
                return;
            }
            ...
            if (isRepeated(tBoxNumberSub))
                if (JOptionPane.showConfirmDialog(this, BadmintonDlgConst.ConfirmMsgOfRepeatedBoxNumber,
    ...
    private String[] initHeader() {
        ...
        try {
            ...
            header[0] = BadmintonDlgConst.BoxNumber;
            header[1] = BadmintonDlgConst.AddUp;
