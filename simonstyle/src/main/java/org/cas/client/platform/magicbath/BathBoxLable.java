...
public class BathBoxLable extends PicturePane implements MouseListener {
    public static int WIDTH = 50;
    public static int HEIGHT = 80;

    public BathBoxLable(int pID, String pSubject, Object pTime) {
        super(PIMPool.pool.getImage(pTime == null ? "/org/cas/client/platform/magicbath/img/Bath1.gif"
                : "/org/cas/client/platform/magicbath/img/Bath2.gif"));
        id = pID;
        subject = pSubject;
        time = pTime;
    ...
    public void mouseClicked(
        ...
        if (e.getClickCount() > 1 && time == null) {
            MagicbathGeneralPanel.pOpenBox.openBox(subject);
            ...
            if (tRec.getFieldValue(ContactDefaultViews.CATEGORY).toString().indexOf("桑拿") > -1)
                MagicbathGeneralPanel.pAddService.addService("", "桑拿", subject, 1);
        }
    }
    ...
    public boolean resetState(
        ...
        if (pSubject.equals(subject)) {
            time = pTime;
            preparePicture(PIMPool.pool.getImage(pTime == null ? "/org/cas/client/platform/magicbath/img/Bath1.gif"
                    : "/org/cas/client/platform/magicbath/img/Bath2.gif"));
            setToolTipText("开台时间:".concat(String.valueOf(time)));
            repaint();
            return true;
