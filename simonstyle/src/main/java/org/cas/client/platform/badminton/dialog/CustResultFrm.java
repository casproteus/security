...
public class CustResultFrm extends JFrame implements ActionListener, FocusListener, ComponentListener {
    ...
    public void actionPerformed(
        ...
        if (s == ok) {
            ...
            if (VIPIdx < 0) {
                fldReceive.setText("");
                fldChange.setText("");
                lblChange.setText(BadmintonDlgConst.Change);
            } else {
                ...
                lblChange.setText(BadmintonDlgConst.MonLeft);
            }
        }
    }
    ...
    private void initComponent() {
        ...
        lblVIPNumber = new JLabel(BadmintonDlgConst.SeleVipNumber);
        cmbVIPNumber = new JComboBox();
        lblReceive = new JLabel(BadmintonDlgConst.Received);
        fldReceive = new JTextField();
        lblChange = new JLabel(BadmintonDlgConst.Change);
    ...
    private void closeBox() {
        for (int j = 0; j < values.length; j++) { // update the records in DB.
            ...
            if (tBoxNumber == null)
                break;
            for (int i = 0, len = BadmintonGeneralPanel.pCalculate.contactSubjectAry.length; i < len; i++)
                if (BadmintonGeneralPanel.pCalculate.contactSubjectAry[i].equals(tBoxNumber)) {
                    PIMRecord tmpRec =
                            CASControl.ctrl.getModel().selectRecord(CustOpts.custOps.APPNameVec.indexOf("Contact"),
                                    BadmintonGeneralPanel.pCalculate.contactIDAry[i], 100); // to select a record from
                                                                                            // DB.
                    ...
                    BadmintonTiedView.getInstance().resetState(tBoxNumber, null);
                    break;
                }
        }

        BadmintonGeneralPanel.pCalculate.initBoxNumbers(); // reinit the ComboBox.
