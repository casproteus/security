...
CONF_HOME="$STATISTICS_HOME/etc"
LIB_HOME="$STATISTICS_HOME/lib"
LOG_HOME="$STATISTICS_HOME/log"

export STATISTICS_HOME CONF_HOME LIB_HOME LOG_HOME
...
# Wrapper
WRAPPER_CMD="$STATISTICS_HOME/bin/linux/$OS_TAG/wrapper"
LIB_NATIVE_HOME="$STATISTICS_HOME/lib/linux/$OS_TAG"
WRAPPER_CONF="../etc/statistics.wrapper.conf"
export LIB_NATIVE_HOME
...
## the assignment below is correct, do not edit -hd
JDK_PATH=%{JDKPath}

if [ -d "$JDK_PATH" ]
then
	echo Using JDK located at $JDK_PATH 
	JAVACMD="$JDK_PATH/bin/java"
else
	if [ -z "$JAVA_HOME" ]
	then
		JAVACMD=java
